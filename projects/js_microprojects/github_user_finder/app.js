...
class Github {
  ...
  drawErr(msgText) {
    ...
    document.getElementById("resultProfile").style =
  ...
  checkErr(response) {
    if (response.status === 404) {
      this.drawErr("User not found!");
    }
    if (response.status === 403) {
      this.drawErr("Request limit exceeded!");
    } else {
      if (!response.ok) {
  ...
  async sendRequestRepos(username) {
    const response = await fetch(
      `https://api.github.com/users/${username}/repos?client_id=002983dd27384f1ff29b&client_secret=5bec329a1eb084057d928ecc80ed1f0933e9f8df`
    );
    const responseData = await response.json();
    return responseData;
  }
...
function UIUser(user) {
  ...
  document.getElementById(
    "companyOutput"
  ).textContent = `Company: ${user.company}`;
  document.getElementById(
    "websiteOutput"
  ).textContent = `Website: ${user.blog}`;
  document.getElementById(
    "locationOutput"
  ).textContent = `Location: ${user.location}`;
  document.getElementById(
    "memberSinceOutput"
  ).textContent = `Member since ${user.created_at}`;
  document.getElementById(
    "publicReposOutput"
  ).textContent = `Public repos: ${user.public_repos}`;
  document.getElementById(
    "publicGistsOutput"
  ).textContent = `Public gists: ${user.public_gists}`;
  document.getElementById(
    "followersOutput"
  ).textContent = `Followers: ${user.followers}`;
  document.getElementById(
    "followingOutput"
  ).textContent = `Following: ${user.following}`;
}
...
function sendRequests(e) {
  ...
  if (inputField.value != "") {
    ...
    request
      .sendRequestUser(inputField.value)
      .then((res) => UIUser(res))
      .catch((err) => console.log(err));

    request
      .sendRequestRepos(inputField.value)
