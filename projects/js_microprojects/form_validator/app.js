...
document
  .querySelector("#phoneText")
  .addEventListener("focus", setNumFocusOnEnd);
document
  .querySelector("#phoneText")
  .addEventListener("input", setNumFocusOnEnd);
document
  .querySelector("#phoneText")
  .addEventListener("click", setNumFocusOnEnd);
document
  .querySelector("#phoneText")
  .addEventListener("keyup", setNumFocusOnEnd);
document.querySelector("#phoneText").addEventListener("keyup", checkIfNum);

function setNumFocusOnEnd(e) {
  numberOutput.focus();
  numberOutput.setSelectionRange(
    numberOutput.value.length,
    numberOutput.value.length
  );
}
...
function checkIfNum(e) {
  ...
  if (
    rePhoneCheat.test(number) ||
    rePhoneBracketsCheat.test(number) ||
    number.length > 12
  ) {
    numberOutput.value = "";
...
function check(e) {
  ...
  if (!reName.test(nickname)) {
    drawErr("Wrong Name");
  }
  if (!rePhone.test(phone)) {
    drawErr("Wrong phone number");
  }
...
function drawErr(msg) {
  ...
  document
    ...
    .insertBefore(drawLine, document.querySelector("#myForm"));
