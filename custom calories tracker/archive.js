...
function saveDay() {
  ...
  if (mealsToday === 0) {
    ...
    function saveMeal(meal) {
      ...
      const theScale =
        meal.firstElementChild.nextElementSibling.nextElementSibling
          .textContent === "g"
          ? 1
          : 2;
      ...
      const theKcal =
        theScale === 1
          ? (meal.firstElementChild.nextElementSibling.nextElementSibling
            ...
            100
          : meal.firstElementChild.nextElementSibling.nextElementSibling
              .nextElementSibling.textContent / theAmount;
...
function archiveDrawDay(day) {
  ...
  if (archiveContainer.innerHTML === "") {
    archiveContainer.appendChild(archiveDay);
  } else {
    archiveContainer.insertBefore(
      archiveDay,
      archiveContainer.firstElementChild
    );
  }

  document
    .querySelector(`#archiveDel${timestamp}`)
    .addEventListener("click", archiveDeleteDay);
  ...
  document
    .querySelector(`#archiveExpand${timestamp}`)
    .addEventListener("click", () => {
      if (
        document.querySelector(`#archiveExpand${timestamp}`).textContent ===
        "Expand..."
      ) {
        document.querySelector(`#archiveTable${timestamp}`).style =
          "display: block";
        document.querySelector(`#archiveExpand${timestamp}`).textContent =
          "Collapse";
      } else if (
  ...
  document.getElementById(`archiveDate${timestamp}`).textContent = `
    ${new Date(day[day.length - 1]).getDate()}.${new Date(
    day[day.length - 1]
  ).getMonth()}.${new Date(day[day.length - 1]).getFullYear()}`;
  ...
  day.forEach((el) => {
    if (typeof el != "string" && el.constructor != Date) {
      ...
      archiveLine.innerHTML = `
            ...
            <td>${
              el.scale === 2 ? el.amount * el.kcal : (el.amount * el.kcal) / 100
            }</td>
            `;
      document
        .getElementById(`archiveList${timestamp}`)
        .appendChild(archiveLine);
      dayKcal +=
        el.scale === 2 ? el.amount * el.kcal : (el.amount * el.kcal) / 100;
    }
  });
